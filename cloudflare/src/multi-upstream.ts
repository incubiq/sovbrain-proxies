    ...
    if ((knownPrefixes as string[]).includes(prefix)) {
      ...
      if (!cfg[upstream]) {
        return {
          ...
          message: `Model prefix '${prefix}:' requested but no ${upstream} upstream is configured on this Worker.`,
        };
      }
...
export async function dispatchModelsMulti(cfg: UpstreamConfig): Promise<Response> {
  ...
  for (const result of results) {
    ...
    if (result.error) {
      ...
      // Surface per-upstream failures so `wrangler tail` shows them, even
      // when other upstreams succeed and the overall response is 200.
