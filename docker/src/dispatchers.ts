 ...
 *
 * Node 20 compatible: uses only Web API globals (fetch, Response, Headers,
 * ReadableStream) which are available globally in Node 20.
 */

import { translateRequestBody, createOpenAITranslatedStream } from "./anthropic-translator.js";
...
export async function forwardChatToOpenAICompat(
  ...
  // makes the outgoing response framing inconsistent with the body bytes —
  // on streaming responses this leaks chunk-size markers and bare \n buffer
  // flushes into the SSE body, corrupting JSON payloads at 4KB boundaries.
  // Observed with Ollama upstream.
